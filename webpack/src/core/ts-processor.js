...
const { stringPlusToTemplateExpression } = require('../utils/ast-node-transform')
...
// function arrayArgsToObject(args: Array<ts.Expression>) {
//   if (args.length === 0) return [];

//   const props: Array<ts.PropertyAssignment> = [];
//   args.forEach((arg, i) => {
//     if (!arg) return;
...
class TsProcessor {
  constructor(sourceFile, options) {
    ...
    this.findReplaceRange(sourceFile)
  }

  findReplaceRange(target) {
    this.sourceFile
    ...
    ts.visitNode(target, nodeVisitor)
    debugger
    return { changeset: this.changeset.changesets }
  }
